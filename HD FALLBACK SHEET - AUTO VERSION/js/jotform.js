...
//                        p.$jot('.jt-content, .jt-title').css('width', 'auto');
    ...
    calculationType: function (id) {
        var paymentTypes = ['control_stripe', 'control_paymill', 'control_payment', 'control_paypal', 'control_paypalexpress', 'control_paypalpro', 'control_clickbank', 'control_2co', 'control_googleco', 'control_worldpay', 'control_onebip', 'control_authnet', 'control_dwolla', 'control_braintree', 'control_square'];
    ...
    isPaymentSelected: function () {
        ...
        if (window.productID) {
            ...
            $H(window.productID).each(function (pair) {
                ...
                if (elem.checked) {
                    // get quantity field
                    if (!!elem.up().select('select,input[id*="_quantity_"]').length) {
                        var quantityField = elem.up().select('select,input[id*="_quantity_"]')[0];
                    }
    ...
    description: function (input, message) {
        ...
        /* Prevent XSS */
        content.insert(message.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;'));
